...
@ClassDescription(label="WorldMags.net")
	...
	public void suggestIssues() throws KioskIssuesSuggesterException {
		for (int page=1; page<MAX_PAGE_TO_GRAB; page++) {
			...
			for (Element magazineElement : elements) {
				...
				MagazineManager.getInstance().suggest( new DownloadSuggestion(title, imageElt.absUrl("src"), url, null, null, -1.0f, getClass(), false, magazineLink.absUrl("href")));
			}
		}
	}
