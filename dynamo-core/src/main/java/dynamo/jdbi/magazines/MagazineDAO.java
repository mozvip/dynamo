...
public interface MagazineDAO {
	...
	@SqlUpdate("MERGE INTO MAGAZINEISSUE(ID, ISSUE, ISSUEDATE, RAWNAME, YEAR, SPECIAL, LANGUAGE, MAGAZINE_SEARCHNAME, CREATION_DATE) VALUES(:id, :issue, :issueDate, :rawName, :year, :special, :language, :searchName, CURRENT_TIMESTAMP)")
	void saveIssue(@Bind("id") long id, @Bind("issue") int issue, @Bind("issueDate") Date issueDate, @Bind("rawName") String rawName, @Bind("year") int year, @Bind("special") boolean special, @BindEnum("language") Language language, @Bind("searchName") String searchName);
	...
	@SqlQuery("SELECT DOWNLOADABLE.*, MAGAZINEISSUE.* FROM MAGAZINEISSUE INNER JOIN DOWNLOADABLE ON MAGAZINEISSUE.ID = DOWNLOADABLE.ID INNER JOIN MAGAZINE ON MAGAZINEISSUE.MAGAZINE_SEARCHNAME = MAGAZINE.SEARCHNAME WHERE DOWNLOADABLE.STATUS='SUGGESTED' AND (MAGAZINE.LANGUAGE =:language OR :language IS NULL) AND UPPER(MAGAZINEISSUE.RAWNAME) LIKE :filter ORDER BY CREATION_DATE DESC")
	@Mapper(MagazineIssueMapper.class)
	public List<MagazineIssue> getKioskContents(@BindEnum("language") Language language, @BindContains("filter") String filter);

	@SqlQuery("SELECT DOWNLOADABLE.*, MAGAZINEISSUE.* FROM MAGAZINEISSUE INNER JOIN DOWNLOADABLE ON MAGAZINEISSUE.ID = DOWNLOADABLE.ID INNER JOIN MAGAZINE ON MAGAZINEISSUE.MAGAZINE_SEARCHNAME = MAGAZINE.SEARCHNAME WHERE DOWNLOADABLE.STATUS='DOWNLOADED' AND (MAGAZINE.LANGUAGE =:language OR :language IS NULL) AND UPPER(MAGAZINEISSUE.RAWNAME) LIKE :filter ORDER BY CREATION_DATE DESC")
	@Mapper(MagazineIssueMapper.class)
	List<MagazineIssue> getCollectionContents(@BindEnum("language") Language language, @BindContains("filter") String filter);

	@SqlQuery("SELECT DOWNLOADABLE.*, MAGAZINEISSUE.* FROM MAGAZINEISSUE INNER JOIN DOWNLOADABLE ON MAGAZINEISSUE.ID = DOWNLOADABLE.ID INNER JOIN MAGAZINE ON MAGAZINEISSUE.MAGAZINE_SEARCHNAME = MAGAZINE.SEARCHNAME WHERE DOWNLOADABLE.STATUS IN('WANTED','SNATCHED') AND (MAGAZINE.LANGUAGE =:language OR :language IS NULL) AND UPPER(MAGAZINEISSUE.RAWNAME) LIKE :filter ORDER BY CREATION_DATE DESC")
	@Mapper(MagazineIssueMapper.class)
